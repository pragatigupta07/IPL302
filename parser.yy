...
std::map<string, vector<string>> funcPrintStr;
...
%token <std::string> MAIN PRINTF IDENTIFIER  
...
%nterm <funcall_astnode*> printf_call;
...
main_definition:
	INT MAIN '(' ')'
	{
		fname = "main";
		st = new SymbTab();
		datatype dt = createtype(1);	//1 for int type
		if(gst.search("main") !=NULL)
		{
			error(@2, "The function main has a previous definition");
			
		}   
		gst.push("main" , new SymbTabEntry("fun","global",dt,0,0,st));	// push the local symbol table in the global one
		
		currFunc = gst.search("main");
	}
	compound_statement
	{
		$$ = $6;
...
printf_call:
	PRINTF '(' STRING_LITERAL ')' ';' {
		if (funcPrintStr.find(fname) == funcPrintStr.end()){
			vector<string> arg;
			arg.push_back($3);
			funcPrintStr[fname] = arg;
		}
		else{
			funcPrintStr[fname].push_back($3);
		}

		exp_astnode* exp_str ;
		exp_str = new stringconst_astnode($3);
		exp_str->lvalue = false;
		exp_str->data_type = createtype(7);
